...
var ControlsList = [BgColorPicker];
...
MainMenu.prototype.notifyStateChange = function notifyStateChange () {
...
var config = {
  timestamp: '2017-05-07T19:07:26.616Z',
  git_rev: 'a414bc4'
};
...
function buildData() {
  ...
  var texture = regl.texture({ data: srcImage, format: 'rgb', flipY: true });
  var w = texture.width;
  var h = texture.height;
...
var glClearColor = [0, 0, 0, 1];
regl.frame(function () {
  if (command === null) {
    return;
  }
  regl.clear({ color: glClearColor });
  command();
});
...
menu.addChangeListener(function (state) {
  console.log(state);
  glClearColor = state.glClearColor;
});
